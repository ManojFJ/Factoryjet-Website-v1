    ...
    <header className={`fixed top-0 left-0 right-0 z-50 flex justify-center transition-all duration-300 ${isScrolled ? 'pt-4' : 'pt-6'}`}>
      ...
      <div
        className={`
          ...
          ${isScrolled
            ? 'w-[90%] md:w-[85%] max-w-6xl bg-white/70 backdrop-blur-xl border border-white/40 shadow-lg shadow-black/5 rounded-full py-3 px-6'
      ...
      >
        ...
        <Link to="/" className="flex items-center gap-2 cursor-pointer group">
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${isScrolled ? 'bg-jet-blue text-white' : 'bg-white text-jet-blue shadow-md'}`}>
            <Rocket size={20} fill="currentColor" className="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
          </div>
          <span className={`text-xl md:text-2xl font-display font-bold tracking-tight transition-colors ${isScrolled ? 'text-jet-navy' : 'text-white'}`}>
            FactoryJet
          </span>
        </Link>
            ...
            <div key={item.label} className="relative group px-3 py-2">
              {renderLink(
                ...
                `flex items-center gap-1 text-sm font-medium transition-colors ${
                  isScrolled ? 'text-gray-600 hover:text-jet-blue' : 'text-gray-200 hover:text-white'
                }`
              )}
        ...
        <div className="lg:hidden">
          <button
            ...
            className={`p-2 rounded-lg transition-colors ${isScrolled ? 'text-jet-navy hover:bg-gray-100' : 'text-white hover:bg-white/20'}`}
          >
