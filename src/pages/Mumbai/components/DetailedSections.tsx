          ...
          {servicesData.map((service, index) => {
            ...
            return (
              <div key={index} className={`bg-white rounded-2xl p-8 shadow-sm hover:shadow-xl transition-shadow border border-light flex flex-col ${isFeatured ? 'lg:col-span-2 lg:bg-navy' : ''}`}>
                ...
                <button className={`w-full py-3 rounded-lg font-bold transition-colors ${isFeatured ? 'bg-secondary text-white hover:bg-orange-600 lg:hover:bg-white lg:hover:text-navy' : 'bg-primary text-white hover:bg-navy'}`}>
               ...
               <>
                 ...
                 <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setFormSubmitted(true); }}>
                   <div>
                     ...
                     <input type="text" required className="w-full border border-light rounded-lg p-3 text-sm focus:ring-2 focus:ring-primary outline-none" placeholder="John Doe" />
                   </div>
                   <div>
                     <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Company Name</label>
                     <input type="text" required className="w-full border border-light rounded-lg p-3 text-sm focus:ring-2 focus:ring-primary outline-none" placeholder="Your Business Ltd." />
                   </div>
                   <div>
                     <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Phone Number</label>
                     <input type="tel" required className="w-full border border-light rounded-lg p-3 text-sm focus:ring-2 focus:ring-primary outline-none" placeholder="+91 99999 99999" />
                   </div>
                   <div>
                     <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Email Address</label>
                     <input type="email" required className="w-full border border-light rounded-lg p-3 text-sm focus:ring-2 focus:ring-primary outline-none" placeholder="john@company.com" />
                   </div>
                   <button type="submit" className="w-full bg-primary hover:bg-navy text-white font-bold py-3 rounded-lg mt-2 transition-colors">
                     Download PDF Report
                   </button>
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-8">
        ...
        <div className="space-y-4">
          {faqData.map((item, index) => (
            <div key={index} className="bg-white rounded-lg border border-light overflow-hidden">
              <button 
              ...
              >
                {item.question}
                {openIndex === index ? <ChevronUp className="w-5 h-5 text-secondary" /> : <ChevronDown className="w-5 h-5 text-slate-400" />}
              </button>
              {openIndex === index && (
