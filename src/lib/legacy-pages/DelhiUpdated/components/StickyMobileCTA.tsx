...
const StickyMobileCTA: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Visible after 20% scroll (approx) or immediate if needed, prompt says >20%
    ...
    const handleBlur = (e: FocusEvent) => {
         // Check if the new focus is also an input, otherwise clear focus state
        setTimeout(() => {
  ...
  // Hide if not scrolled enough, or if user is typing in a form, or on desktop
